...
const apiUrl = "https://603837394e3a9b0017e92dea.mockapi.io/people/";
...
const information = document.querySelector("form")
...
function pullinformation(singlePerson){
  return `
    <li data-id="${singlePerson.id}">
      <h3 role="button">${singlePerson.fullName}</h3>
      <button>X</button>
    </li>
  `;
...
function sendData(e){
  ...
  //Creacion del objeto donde agregamos todos los valores. 
  const personInfo = { 
    "fullName": `${information.elements[0].value}`, 
    "email":`${information.elements[1].value}`, 
    "phoneNumber":`${information.elements[2].value}`, 
    "city":`${information.elements[3].value}`,
  }

  fetch(apiUrl, {
    method: "POST",

    //Darle a la orden a la API que se extraiga la informacion en formato JSON
    headers: {
    ...
    .then((person) => {
      htmlListElement.innerHTML += pullinformation(person);
    })
    .catch((err) => {
      console.error(err);
    });
...
// Agregamos en el eventoo de escuchar al submit del formulario
information.addEventListener("submit", sendData);